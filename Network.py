...
class Network:
    ...
    def load_network(self,model,device='CPU',cpu_extenstion=None,num_requests=1):
        ...
        if 'CPU' in device:
            ...
            unsupported_layers = [l for l in self.network.keys() if l not in supported_layers]
        ...
        # get input shape
        self.input_shape = self.network[self.input_blob].shape
    ...
    def async_infer(self,image,request_id=0):
        ...
        if self.status == 0:
            self.inference_request = self.exec_network.start_async(inputs={self.input_blob:image},request_id=request_id)
